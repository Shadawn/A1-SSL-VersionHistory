 //Для механизма А1БСПИсторияИзменений.
&НаСервере
...
Процедура А1БСП_ЗаполнитьТипыОбъектовВДеревеЗначений()
	...
	//А1Начало
	ОбъектыМеханизма = А1Э_Механизмы.ОбъектыМеханизма("А1БСПИсторияИзменений");
	Если ОбъектыМеханизма <> Неопределено Тогда
	...
	Для Каждого Тип Из МассивТипов Цикл
		Если Тип = Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных") Тогда
			Продолжить;
		КонецЕсли;
		//А1Начало - меняем условия чтобы работали по объекту метаданных, иначе не хочет добавлять расширения.
		МетаданныеОбъекта = Метаданные.НайтиПоТипу(Тип);
		ТипМетаданных = А1Э_Строки.Перед(МетаданныеОбъекта.ПолноеИмя(), ".");
		...
		ИначеЕсли ТипМетаданных = "ПланРасчета" Тогда
